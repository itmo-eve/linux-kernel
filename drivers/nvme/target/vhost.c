...
static int nvmet_vhost_parse_admin_cmd(struct nvmet_req *req)
{
	...
	pr_warn("nvmet_vhost_parse_admin_cmd GET OPCODE=[%d]", cmd->common.opcode);
			 ...
			 struct vhost_nvme_target *nvme_tgt)
{
	...
	pr_warn("SET ENDPOINT kernel-nvme-vhost", cmd->common.opcode);
...
static int nvmet_vhost_ioc_bar(struct nvmet_vhost_ctrl *ctrl, void __user *argp)
{
	...
	pr_warn("nvmet_vhost_ioc_bar  bar.type=[%d], bar.offset=[%llu]", bar.type, bar.offset);
			      ...
			      unsigned long arg)
{
	...
	pr_warn("nvmet_vhost_ioctl Get ioctl [%d]", ioctl);
	switch (ioctl) {
